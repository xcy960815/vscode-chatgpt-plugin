window.onload = function () {
  ...
  const aiSvg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 41 41" fill="none" stroke-width="1.5" class="w-5 mr-2 w-4 h-4">
  ...
  const userSvg = `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="mr-2 w-4 h-4"><path stroke-linecap="round" stroke-linejoin="round" d="M17.982 18.725A7.488 7.488 0 0012 15.75a7.488 7.488 0 00-5.982 2.975m11.963 0a9 9 0 10-11.963 0m11.963 0A8.966 8.966 0 0112 21a8.966 8.966 0 01-5.982-2.275M15 9.75a3 3 0 11-6 0 3 3 0 016 0z" /></svg>`;

  const copyButtonSvg = `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="mr-1 w-3 h-3"><path stroke-linecap="round" stroke-linejoin="round" d="M15.666 3.888A2.25 2.25 0 0013.5 2.25h-3c-1.03 0-1.9.693-2.166 1.638m7.332 0c.055.194.084.4.084.612v0a.75.75 0 01-.75.75H9a.75.75 0 01-.75-.75v0c0-.212.03-.418.084-.612m7.332 0c.646.049 1.288.11 1.927.184 1.1.128 1.907 1.077 1.907 2.185V19.5a2.25 2.25 0 01-2.25 2.25H6.75A2.25 2.25 0 014.5 19.5V6.257c0-1.108.806-2.057 1.907-2.185a48.208 48.208 0 011.927-.184" /></svg>`;

  const copiedButtonSvg = `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="mr-1 w-3 h-3"><path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5" /></svg>`;

  const newTabButtonSvg = `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="mr-1 w-3 h-3"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" /></svg>`;

  const insertButtonSvg = `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="mr-2 w-3 h-3"><path stroke-linecap="round" stroke-linejoin="round" d="M11.25 4.5l7.5 7.5-7.5 7.5m-6-15l7.5 7.5-7.5 7.5" /></svg>`;
  ...
  console.log('questionInputElement', questionInputElement);
  ...
  const handleAddQuestion = (messageOption) => {
    ...
    answerListElement.innerHTML += `<div class="p-4 self-end mt-2 question-element relative input-background">
                        <h3 class="mb-5 mt-0 flex items-center">${userSvg} You</h3>
                        <no-export class="mb-2 flex items-center">
                            <button title="${editButtonTitle}" id="edit-button" class="p-1.5 flex items-center rounded-lg absolute right-6 top-6">${editButtonSvg}</button>
                            <div class="hidden send-cancel-container flex gap-2">
                                <button title="${sendButtonTitle}" id="send-button" class="send-button p-1 pr-2 flex items-center rounded-md">${sendButtonSvg}&nbsp;${sendButtonName}</button>
                                <button title="${cancelButtonTitle}" id="cancel-button" class="cancel-button p-1 pr-2 flex items-center rounded-md">${cancelButtonSvg}&nbsp;${cancelButtonName}</button>
                            </div>
                        </no-export>
  ...
  const handleAddAnswer = (messageOption) => {
    ...
    if (existingMessageElement) {
      ...
      answerListElement.innerHTML += `<div class="p-4 self-end mt-4 pb-8 answer-element-ext">
                        <h3 class="mb-5 flex items-center">${aiSvg}ChatGPT</h3>
  ...
  const handleAddError = (messageOption) => {
    ...
    answerListElement.innerHTML += `<div class="p-4 self-end mt-4 pb-8 error-element-ext">
                        <h2 class="mb-5 flex items-center">${aiSvg}ChatGPT</h2>
  ...
  // 点击复制按钮
  const handleClickCopyButton = (targetButton) => {
    navigator.clipboard
      ...
      .then(() => {
        ...
        targetButton.innerHTML = `${copiedButtonSvg} ${copiedButtonName}`;
  ...
  // 点击插入按钮
  const handleClcikInsertButton = (targetButton) => {
    postMessageToVscode({
      type: 'insert-code',
  ...
  // 点击新开tab按钮
  const handleClickNewTabButton = (targetButton) => {
    postMessageToVscode({
      type: 'open-newtab',
      value: targetButton.parentElement?.nextElementSibling?.lastChild?.textContent,
    });
  ...
  // 点击编辑按钮
  const handleClickEditButton = (targetButton) => {
    // 获取到当前的问题元素
  ...
  // 点击重发按钮
  const handleClickSendButton = (targetButton) => {
    const questionElement = targetButton.closest('.question-element');
  ...
  // 点击取消按钮
  const handleClickCancelButton = (targetButton) => {
    const questionElement = targetButton.closest('.question-element');
    const sendAndCancelContainer = targetButton.closest('.send-cancel-container');
    const resendElement = targetButton.parentElement.parentElement.firstElementChild;
    sendAndCancelContainer.classList.add('hidden');
  ...
  document.addEventListener('click', (e) => {
    ...
    // 点击设置按钮
    if (targetButton?.id === 'update-settings-button') {
      postMessageToVscode({
        type: 'open-settings',
      });
      return;
    ...
    // 点击更新apikey按钮
    if (targetButton?.id === 'update-key-button') {
      postMessageToVscode({
        type: 'update-key',
      });
      return;
    ...
    // 点击设置提示按钮
    if (targetButton?.id === 'settings-prompt-button') {
      postMessageToVscode({
    ...
    // 点击编辑按钮
    if (targetButton?.id === 'edit-button') {
      handleClickEditButton(targetButton);
      return;
    }
    // 点击重发按钮
    if (targetButton?.id === 'send-button') {
      handleClickSendButton(targetButton);
      return;
    }
    // 点击取消按钮
    if (targetButton?.id === 'cancel-button') {
      handleClickCancelButton(targetButton);
      return;
    }

    // 点击复制按钮
    if (targetButton?.id === 'copy-button') {
      handleClickCopyButton(targetButton);
      return;
    }
    // 点击插入按钮
    if (targetButton?.id === 'insert-button') {
      handleClcikInsertButton(targetButton);
      return;
    }
    // 点击新标签按钮
    if (targetButton?.id === 'new-tab-button') {
      handleClickNewTabButton(targetButton);
      return;
    }
  });
};
